...
int connectRemoteServer(char* host_addr, int port_num)
{
	...
	bzero((char*)&server_addr, sizeof(server_addr)); // The bzero function is used to set all bytes of the server_addr structure to zero
	server_addr.sin_family = AF_INET; //This line sets the address family of the socket address to AF_INET, which indicates that the address is an IPv4 address.
	server_addr.sin_port = htons(port_num); //Here, htons is used to convert the port number port_num from host byte order to network byte order (big-endian). The port number is then assigned to server_addr.sin_port.

	bcopy((char *)host->h_addr,(char *)&server_addr.sin_addr.s_addr,host->h_length);//This line copies the server's IP address from the host structure (host->h_addr) to server_addr.sin_addr.s_addr. The host->h_addr is
	// a pointer to the first address in the list of the server's addresses. bcopy is used here to copy host->h_length bytes from host->h_addr to server_addr.sin_addr.s_addr
...
int handle_request(int clientSocket, ParsedRequest *request, char *tempReq)
{
	...
	*/
	...
	//This loop effectively relays data between the client and the remote server, buffering the data in temp_buffer before sending it to the client.
	while(bytes_send > 0)
	{
